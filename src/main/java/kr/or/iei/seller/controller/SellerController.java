...
public class SellerController {
	...
	public String addNewProduct(Product p, String[] optionColor, String[] optionSize,
			@SessionAttribute(required = false) Seller s, MultipartFile upfile, Model model) {
		...
		/*
		 * for(int i=0;i<optionColor.length;i++) { ProductOption productOption = new
		 * ProductOption(); productOption.setOptionColor(optionColor[i]);
		 * productOption.setOptionSize(optionSize[i]); //productOption.setProductNo(2에서
		 * 조회한 번호); //insert }
		 */
	...
	// 취소환불 고객 신청 > 판매자 취소환불내역으로 이동
	@GetMapping(value = "/cancelRefundBtn")
	 ...
	 * @ResponseBody
	 * 
	 * @PostMapping(value="/editor",produces = "plain/text;charset=utf-8") public
	 * String editorUpload(MultipartFile file) { String savepath = root+"editor/";
	 * String filepath = fileUtil.getFilepath(savepath, file.getOriginalFilename());
	 * File image = new File(savepath+filepath); try { file.transferTo(image);
	 * //System.out.println(image); } catch (IllegalStateException | IOException e)
	 * { e.printStackTrace(); } return "/editor/"+filepath; }
	 */
	...
	@GetMapping(value = "/sellingDropDown")
	public String updateOrderState(int orderState, int orderNo, Model model) {
	
		int result = sellerService.updateOrderState(orderState,orderNo);
		if (result > 0) {
			model.addAttribute("title", "변경 완료");
			model.addAttribute("msg", "변경 완료되었습니다.");
			model.addAttribute("icon", "success");
			model.addAttribute("loc", "/seller/selling");
			return "common/msg";
		} else {
			model.addAttribute("title", "변경 실패");
			model.addAttribute("msg", "변경 실패하였습니다.");
			model.addAttribute("icon", "error");
			model.addAttribute("loc", "/");
			return "common/msg";
		}
